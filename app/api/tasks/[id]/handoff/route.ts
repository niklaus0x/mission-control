...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (task.assignedAgentId === input.toAgentId) return NextResponse.json({ error: 'Cannot handoff to the same agent' }, { status: 400 });
    const updatedTask = handoffTask(params.id, task.assignedAgentId, input);
